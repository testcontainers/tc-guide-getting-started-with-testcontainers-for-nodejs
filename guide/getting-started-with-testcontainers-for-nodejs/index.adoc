...
Are you seeing the benefit of testing with a real Postgres instance yet? If not, try changing the `id` type of the `customer` table from `INT` to `BIGINT` and re-run the test, what would you expect to happen? If you had a unit test, or used a mock instead of using Testcontainers, you would _not_ have caught this bug.

Sometimes it's useful to see what Testcontainers is doing in the background. Did we really spin up a container? What version did we use? What's Postgres logging? You can see all this and more by setting the `DEBUG` environment variable. Let's re-run the test with `DEBUG=testcontainers*` and find out:
