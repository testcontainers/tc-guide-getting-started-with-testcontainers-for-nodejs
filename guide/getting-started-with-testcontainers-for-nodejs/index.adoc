...
The `beforeAll` block sets up a real Postgres container. We then initialize a Postgres client from the `pg` library, and connect it to the Postgres instance running in the container. As part of the test setup, we create the Customer table.
...
Perhaps that was a bit too transparent, did we really spin up a container? You can see exactly what Testcontainers is doing by setting the `DEBUG` environment variable. Let's re-run the test with `DEBUG=testcontainers*`:
...
That's a lot of information, you can also see the logs emitted by Postgres. This is very useful when working with more complex, custom setups.

== Conclusion

We have explored how to use Testcontainers for NodeJS to test a NodeJS
